...
USEDLL int BCD_Inspection(TeachBarcode TeachBar, RstBarocde *RstBar = NULL);

USEDLL int BCD_ModuleProcessInput(UCHAR * Src, int imgSizeX, int imgSizeY, POINT st, POINT ed, int type, int color,
	wchar_t* sModuleBarcodeNum, int nFlipX, int nFlipY, int nCheckSum, int nCount, int nThresholdType);
...
USEDLL void BCD_Open_Module(int FovSizeX, int FovSizeY, BOOL bDTKOn, int MachineType, int nModuleNum);