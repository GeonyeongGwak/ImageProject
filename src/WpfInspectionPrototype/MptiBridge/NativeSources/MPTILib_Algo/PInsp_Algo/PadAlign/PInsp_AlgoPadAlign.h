...
class IPINSP_ALGO;
class CPInsp_AlgoPadAlign : public IPINSP_ALGO
{
private:

public:
	CPInsp_AlgoPadAlign(void);
	virtual ~CPInsp_AlgoPadAlign(void);
	...
	BOOL GetROILRTB(UCHAR* ucArrImg, int nImgW, int nImgH, POINTF* ptrpoPoint);
	BOOL GetBWImageSize(UCHAR* ucArrImg, int nImgW, int nImgH, POINTF poMinMax, POINTF* poTL, POINTF* poSize);
	BOOL InspPadAlign(const InspAlgo &sInspAlgo, WndAlgoImg &sWndAlgoImg, InspRoiImgBuf &sInspImageData, RstAlgoPadAlign *sRstAlgo, UCHAR* ucArrDstImg, bool bTeachData, int nAlignCnt, AlignResult * sAlignRes, cv::Rect wndROI);
