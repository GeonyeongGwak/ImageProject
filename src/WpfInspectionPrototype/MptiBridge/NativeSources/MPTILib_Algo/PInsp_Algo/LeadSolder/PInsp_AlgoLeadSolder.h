...
class IPINSP_ALGO;
class CPInsp_AlgoLeadSolder : public IPINSP_ALGO
{
private:

public:
	CPInsp_AlgoLeadSolder(void);
	virtual ~CPInsp_AlgoLeadSolder(void);
	...
	int InspACLead(AngleColorBase sAlgo, InspRoiImgBuf &sInspImg, WndAlgoImg &sWndAlgo, int nDir, int nLeadTipPos, UCHAR *pucRstAC, RstInspAC *sRstInspAC, bool bTeach);
	BOOL InspSolder(const InspAlgo &sInspAlgo, WndAlgoImg &sWndAlgoImg, InspRoiImgBuf &sInspImgBuf, UCHAR* ucArrDstImg, RstAlgoLeadSolder * sRstAlgo = NULL, int nLeadTipPos = -1, byte bViewCJ = 0, UCHAR* ucArrDstImgAC = NULL);
