...
class IPINSP_ALGO;
class CPInsp_AlgoPadBW : public IPINSP_ALGO
{
private:

public:
	CPInsp_AlgoPadBW(void);
	virtual ~CPInsp_AlgoPadBW(void);
	...
	std::map<CString, cv::Mat> m_vArrModelList[2];
	void ModelClear(int nLane);
	bool PreAllocModel(int nLane, CString sPath, cv::Mat& Mask);
	bool FindPreAllocMask(AlgoPadBW* algo, cv::Mat& Mask);
};
