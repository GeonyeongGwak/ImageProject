...
class IPINSP_ALGO;
class CPInsp_AlgoFoot : public IPINSP_ALGO
{
private:
...
public:
	...
	CPInsp_AlgoFoot(void);
	virtual ~CPInsp_AlgoFoot(void);
	...
	virtual cv::Mat FillPolygonImg(cv::Mat ucMask, cv::Mat binImg, int width, int height, POINT* ptDstPoint, bool isExceptROI = false, int nUsedPolygon = 0, int* cnt = nullptr, UCHAR * ucArrOverlapImg = nullptr, int nChanel = 1, int nExcept = 1);
