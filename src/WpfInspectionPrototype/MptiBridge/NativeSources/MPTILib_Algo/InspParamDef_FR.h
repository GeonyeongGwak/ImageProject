...
enum FR_DATA
{
	...
	FR_DATA_NotSetFov2D = 0x20000000,
	FR_DATA_Not_Mix_FR = 0x40000000,
};
enum FR_DATA2
{
...
typedef struct tagAForeignResult
{
	...
	float m_fCenterX;	// Board 상의 좌표 X (mm)
	float m_fCenterY;	// Board 상의 좌표 Y (mm)
	float m_fSizeX;		// 이물 사이즈 X	(mm)
	float m_fSizeY;		// 이물 사이즈 Y	(mm)
	float m_fArea;		// 이물 면적		(m^2)
	float m_fHeight;	// 이물 평균 높이	(um)
	float m_fHeightMax;
	int	  m_nForeignDefect;
	int	  m_nUVColorSelect;
	int	  m_nModuleID;

	void Clone(tagAForeignResult &InspForeignRes)
	{
		InspForeignRes.m_nOPT = m_nOPT;
		InspForeignRes.m_nRange3D = m_nRange3D;
		InspForeignRes.m_fArea_T = m_fArea_T;
		InspForeignRes.m_fAreaMax_T = m_fAreaMax_T;
		InspForeignRes.m_fHeighMin_T = m_fHeighMin_T;
		InspForeignRes.m_fHeighMax_T = m_fHeighMax_T;

		InspForeignRes.m_rcRect = m_rcRect;
		InspForeignRes.m_fCenterX = m_fCenterX;
		InspForeignRes.m_fCenterY = m_fCenterY;
		InspForeignRes.m_fSizeX = m_fSizeX;
		InspForeignRes.m_fSizeY = m_fSizeY;
		InspForeignRes.m_fArea = m_fArea;
		InspForeignRes.m_fHeight = m_fHeight;
		InspForeignRes.m_fHeightMax = m_fHeightMax;
		InspForeignRes.m_nForeignDefect = m_nForeignDefect;
		InspForeignRes.m_nUVColorSelect = m_nUVColorSelect;
		InspForeignRes.m_nModuleID = m_nModuleID;
	}
}AForeignResult;
